...
#parameters
#tf.flags.DEFINE_integer("max_len",100,"the max length of sentence")
...
def BatchLoss(text_length,input_x, input_y, word_index_dict_size,
    ...
    """
    loss,accuracy = text_cnn.inference(input_x = input_x,
                                       input_y = input_y,
...
def DevLoss(dev_x, dev_y, word_index_dict_size, dropout_keep_prob,
    ...
    x_size = len(y)
    dev_loss, dev_acc = BatchLoss(dev_x, dev_y,
             word_index_dict_size, dropout_keep_prob, embedding_dim,
...
def Train(train_path, dev_path, \
    ...
    with g1.as_default(),tf.device("/gpu:" + str(gpus[0])):
        ...
        train_op, average_loss, average_accuracy = MultiGPUCalcLossAccu(
                ...
                cpus)
        dev_loss, dev_acc = DevLoss(dev_x, dev_y,
                word_index_dict_size, dropout_keep_prob, embedding_dim,
	...
	'''
        ...
        loss_summary =tf.summary_scalar("loss", average_loss)
        acc_summary =tf.summary_scalar ("accuracy", average_accuracy)
        dev_loss_summary = tf.summary_scalar("loss", dev_loss)
        dev_acc_summary =tf.summary_scalar ("accuracy", dev_acc)
        ...
        for epoch in range(num_epochs):
		    ...
		    print("1111")
                    DevStep(sess, dev_feed_dict, dev_summary_op, dev_loss,
