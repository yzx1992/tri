  ...
  if use_attention:
      ...
      def attention(query):
        ...
        for i in xrange(num_heads):
          with variable_scope.variable_scope("Attention_%d" % i):
            y = tf.nn.rnn_cell._linear(query, attention_vec_size, True)
      ...
      for i in xrange(len(encoder_outputs)):
        ...
        if i == 0:
          with variable_scope.variable_scope("Initial_Decoder_Attention"):
            initial_state = tf.nn.rnn_cell._linear(encoder_state, output_size, True)
        ...
        with variable_scope.variable_scope("AttnRnnOutputProjection"):
          logit = tf.nn.rnn_cell._linear(output, num_decoder_symbols, True)
    ...
    with variable_scope.variable_scope(scope or "non-attention_RNN"):
      ...
      for time, input_ in enumerate(encoder_outputs):
        ...
        generate_logit = lambda: tf.nn.rnn_cell._linear(encoder_outputs[time], num_decoder_symbols, True)
